...
class Document(list):
    ...
    def load_file(self, filename):
        ...
        # [TODO] 
        # priority: high
        # There are no scanner for v0.2!
        # Use the python default xml library if possible.
        import scanner
        fh = open(filename)
...
# [NOTE]
# This is the old loader. I need it to load the v01 file format in 
class Loader:
...
# The document to migrate v01 to v02
class Document02(Document):
    
    def __init__(self, page, buffer):
        Document.__init__(self, page, buffer)

    def load_file(self, filename):
        assert os.path.isfile(filename)
        self.filepath, self.filename = os.path.split(filename)

        from rayuela import scanner
        fh = open(filename)
        scanner.scan(fh.read(), Loader(self))
